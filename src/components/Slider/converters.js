const closest = (array, n) => {  // hàm lấy vị trí phần tử của mảng
  let minI = 0;
  ...
  if (array[minI] > n) {
    ...
    while (closestIndex === null) {  // thuật toán tìm kiếm nhị phân
      const midI = Math.round((minI + maxI) / 2);
      const midVal = array[midI];

      if (midVal === n) {
        closestIndex = midI;
      } else if (maxI === minI + 1) { // nếu 2 vị trí cạnh nhau thì vị trí nào gần n hơn thì sẽ return vị trí đó.
...
const valueToPosition = ( // lấy vị trí của value
  value,
  valuesArray,
  sliderLength,
) => {
  ...
  const output =
    (sliderLength * validIndex) / arrLength;
